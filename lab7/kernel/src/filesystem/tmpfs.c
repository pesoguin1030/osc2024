...
int register_tmpfs()
{
    ...
    fs.setup_mount = tmpfs_setup_mount; //將掛載的callback設定為tmpfs_setup_mount
    return register_filesystem(&fs);
}

int tmpfs_setup_mount(struct filesystem *fs, struct mount *_mount) //設置掛載點的文件系統並創建根目錄的 vnode。
{
    // set struct mount 
    _mount->fs = fs;
    // create root vnode
