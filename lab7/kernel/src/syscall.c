...
{       
    ...
    //child_thread->context.fp += child_thread->kernel_sp - curr_thread->kernel_sp; // move fp
    //child_thread->context.sp += child_thread->kernel_sp - curr_thread->kernel_sp; // move kernel sp
    unlock();
...
//         //return tpf->x0;
//         long bytes = vfs_write(curr_thread->fdt[fd], buf, count);
//         return bytes;
//     }
