...
void sd_mount(const char* mountpoint) {
  // check the MBR signature in the first sector of a device by reading the
  // first sector of the device and inspect the last two bytes.
  mbr_t mbr;
  sd_readblock(0, (void*)&mbr);
  if (mbr.signature[0] != 0x55 || mbr.signature[1] != 0xAA) {
    uart_puts("MBR signature check fails");
    return;
  }

  mbr_partition_entry* partition1 = &mbr.partitions[0];
  if (partition1->partition_type == 0x0B) {  // FAT32 with CHS addressing
    fat32_bootsector fat32_bootsec;
    sd_readblock(partition1->first_sector_lba, (void*)&fat32_bootsec);
    fat32fs_init_metadata(partition1, &fat32_bootsec);
    filesystem* fat32fs = fat32fs_init();
    register_filesystem(fat32fs);
    vfs_mount(mountpoint, "fat32fs");
  }
}