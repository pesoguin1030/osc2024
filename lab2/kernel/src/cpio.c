...
int cpio_newc_parse_header(struct cpio_newc_header *this_header_pointer, char **pathname, unsigned int *filesize, char **data, struct cpio_newc_header **next_header_pointer)
{
    ...
    *data = (char *)this_header_pointer + offset;
    ...
    if (*filesize == 0) // Check if the size of the current file is zero
    {
        // If the file size is zero, set the next header pointer to the beginning of the current file data
        *next_header_pointer = (struct cpio_newc_header *)*data;
    }
    else // If the file size is not zero
    {
        offset = *filesize; // Set the offset to the size of the file
        // Calculate the position of the next header, ensuring it is aligned on a 4-byte boundary
        // If the file size is already a multiple of 4, use the file size as the offset
        // If not, increase the offset to the nearest multiple of 4
