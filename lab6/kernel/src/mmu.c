...
void *set_2M_kernel_mmu(void *x0)
{
    ...
    for (int i = 0; i < 512; i++)
    {
        ...
        // every 2MB -> 3 level
        if ((0x00000000 + (0x200000L) * i) >= 0x3F000000L) //timer 相關中斷設備
            ...
            // 0booooiiii Normal memory, oooo = 0b0100 Outer Non-cacheable, iiii  = 0b0100 Inner Non-cahceable 關L1 L2 Cache
    // 0x3c0000000 ~ 0x3f0000000 is GPU (mailbox ..etc) memory
    }

    // setting up second PMD which maps to  0x40000000 ~ 0x7fffffff (1GB) //timer 相關中斷設備
...
void map_one_page(size_t *virt_pgd_p, size_t va, size_t pa, size_t flag)
{
    ...
    for (int level = 0; level < 4; level++)
    {
        ...
        {   
            // setting page
            table_p[idx] = pa;
            table_p[idx] |= PD_ACCESS | PD_TABLE | (MAIR_IDX_NORMAL_NOCACHE << 2) | PD_KNX | flag; //page 也是存11
            return;
        }
...
{
    uart_printf("[Permission fault]: Kill Process\r\n");
    thread_exit();
}