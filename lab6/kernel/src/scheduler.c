...
int exec_thread(char *data, unsigned int filesize)
{
    ...
    add_vma(t, 0x3C000000L, 0x3000000L, 0x3C000000L, 3, 0);//GPU周邊, wr                                                     // device 可能是因為mailbox要讓el0可以執行
    add_vma(t, 0xffffffffb000, 0x4000, (size_t)VIRT_TO_PHYS(t->user_sp), 7, 1);  //user stack xwr                               // stack
    add_vma(t, 0x0, filesize, (size_t)VIRT_TO_PHYS(t->data), 7, 1);                                             // text
    add_vma(t, USER_SIG_WRAPPER_VIRT_ADDR_ALIGNED, 0x2000, (size_t)VIRT_TO_PHYS(signal_handler_wrapper), 5, 0); // for signal wrapper, x r

    t->context.ttbr0_el1 = VIRT_TO_PHYS(t->context.ttbr0_el1); //被設為實體位置
    ...
    __asm__ __volatile__("msr tpidr_el1, %0\n\t"
                         ...
                         "mov sp, %3\n\t"
                         "mov fp, sp\n\t"
                         "dsb ish\n\t" // ensure write has completed, data sync barrier, 確保跟Memory相關的指令都已經執行完畢
                         "msr ttbr0_el1, %4\n\t"
                         // need memory barriers 
                         // gurantee previos instructions are finished
                         // TLB invalidation -> old value are staled
                         "tlbi vmalle1is\n\t" // invalidate all TLB entries, 確保el0 跟 el1 的tlb都被invalidate
                         "dsb ish\n\t"        // ensure completion of TLB invalidatation
                         "isb\n\t"            // clear pipeline 確定前面的指令都執行完畢
                         "eret\n\t" ::"r"(&t->context),
                         "r"(t->context.lr), "r"(t->context.sp), "r"(t->kernel_sp + KSTACK_SIZE), "r"(t->context.ttbr0_el1)); //跳到0x0, 因為ttbr0已經設定好了, 所以可以正確執行
