...
int exec(trapframe_t *tpf, const char *name, char *const argv[])
{
    ...
    // curr_thread->datasize = get_file_size("fork_test.img");
    // char *new_data = get_file_start("fork_test.img");
...
int fork(trapframe_t *tpf)
{
    ...
    //don't copy data into new process, child and parent use same data space
    ...
    store_context(get_current());//get_current()是parent的context, 因為這邊有跳function, 所以已經把lr設為下一行(pc+4), context switch ret 的時候 child 會從下行開始執行
    ...
    unlock();//parent做完才會unlock
    tpf->x0 = newt->pid;
    return newt->pid;   // pid = new

child:

    //uart_sendline("in child\n");
...
{
    //unsigned long signal_ustack = tpf->sp_el0 % USTACK_SIZE == 0 ? tpf->sp_el0 - USTACK_SIZE : tpf->sp_el0 & (~(USTACK_SIZE - 1));//找到 tpf的
    unsigned long signal_ustack;
    if (tpf->sp_el0 % USTACK_SIZE == 0) {
        // 如果 tpf->sp_el0 是 USTACK_SIZE 的整數倍
        signal_ustack = tpf->sp_el0 - USTACK_SIZE;
    } else {
        // 如果 tpf->sp_el0 不是 USTACK_SIZE 的整數倍
